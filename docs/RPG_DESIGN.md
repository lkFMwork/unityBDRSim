...
Gatekeeper Gauntlet, the explorable city, and the office with mentors. Remaining:

1. **Quest system** — objectives, a quest log + HUD tracker, and rewards that
   thread the world, calls, duels, and mentors together.
2. **Promotions & events** — rank-up ceremony, rival/leaderboard, streaks,
   achievements, random events, and "boss" accounts.
3. **AAA presentation** — rigged characters + Animator (the `AvatarBuilder.SetConfig`
